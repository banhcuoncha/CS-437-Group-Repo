...
class MyAwsGreengrassV2Component():
    ...
    def __init__(self, ggv2_component_config):
        ...
        self.my_sensor_message_handler = MySensorMessageHandler(self.publish_message, self.publish_error, self.message_formatter, self.publish_to_custom_topic)
    ...
    ### Adding new custom message topic 
    def publish_to_custom_topic(self, topic, message):
        '''
        Publishes a given message to a custom MQTT topic directly.
        '''
        try:
            payload = json.dumps(message)
            self.pubsub_client.mqtt_client.publish(topic, payload, 0)
            log.info(f"[Custom MQTT] Published message to topic '{topic}': {payload}")
        except Exception as e:
            log.error(f"[Custom MQTT] Failed to publish to {topic}: {e}")
...
if __name__ == "__main__":
    ...
    try:
        if len(sys.argv) < 2:
            raise ValueError("No configuration argument passed to main.py")

        config_str = sys.argv[1]
        ggv2_component_config = json.loads(config_str)

        log.info(f"GGV2 config received: {ggv2_component_config}")

        ggv2_component = MyAwsGreengrassV2Component(ggv2_component_config)
        ggv2_component.service_loop()

    except Exception as err:
        log.error(f"Exception occurred initializing component: {err}", exc_info=True)